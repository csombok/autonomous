    ...
    class ViewportFactory
    {
        ...
        public IEnumerable<ViewportWrapper> CreateViewPorts(IEnumerable<GameObject> objects, CameraSetup cameraSetup)
        {
            ...
            if (numViewPorts == 1)
            {
                ...
                yield return CreateViewPort(objects.ElementAt(0), 0, 0, width, height, cameraSetup);
            }
            ...
            if (numViewPorts == 2)
            {
                ...
                yield return CreateViewPort(objects.ElementAt(0), 0, 0, width, height, cameraSetup);
                yield return CreateViewPort(objects.ElementAt(1), 0, height, width, height, cameraSetup);
            }
            ...
            if (numViewPorts >= 3)
            {
                ...
                yield return CreateViewPort(objects.ElementAt(0), 0, 0, width, height, cameraSetup);
                yield return CreateViewPort(objects.ElementAt(1), 0, height, width, height, cameraSetup);
                yield return CreateViewPort(objects.ElementAt(2), width, 0, width, height, cameraSetup);
                if (objects.Count()>3)
                    yield return CreateViewPort(objects.ElementAt(3), width, height, width, height, cameraSetup);
            }

        }

        static ViewportWrapper CreateViewPort(GameObject gameObject, int x, int y, int width, int height, CameraSetup cameraSetup)
        {
            if (gameObject is FinishLine)
                return new FinishLineViewport(x, y, width, height, gameObject);
            return new GameObjectViewport(x, y, width, height, gameObject, cameraSetup);
        }
    }
}
